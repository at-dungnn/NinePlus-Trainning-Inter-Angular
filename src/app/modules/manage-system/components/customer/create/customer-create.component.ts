  ...
  onSubmit() {
    ...
    if (this.formAddNewCustomer.valid) {
      ...
      this.customerService.getListCustomer().subscribe(customers => {
        // check id customer
        let customer: Customer | undefined = customers.find(item => {
          return item.id === this.valueFormAddNewCustomer.id
        })
        if (!_.isEmpty(customer)) {
          this.showError("Id already exist")
        } else {
          this.saveCustomer(this.valueFormAddNewCustomer)
        }
      })
    }
  }
